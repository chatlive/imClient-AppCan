		...
		<div class="baseheader">
			<div class="ui secondary menu">
				...
				<div class="center menu">
					<div class="item">
						<slot name="head_title">群组成员-删除</slot>
					</div>
				</div>
			...
			<slot name="body_content">
				<div>
					<mt-cell :title="checkedCount">
						<mt-button size="small" type="danger" @click.native="save">删除</mt-button>
					</mt-cell>
				</div>
	...
	export default {
		...
		methods: {
			save: function() {
				...
				CHAT.GroupManage.gM_RemoveUsers(model);
				let groupInfo = this.saveBefore(model.groupId, model.accounts);
				this.memberRemove(groupInfo);
				this.backOff();
			},
			saveBefore: function(groupId, accounts) {
				...
				for(let account of accounts) {
					let member = this.members.find(item => item.account === account);
			...
			...mapMutations({
				...
				memberRemove: IMCHATTYPES.IMCHAT_MUTATION_MEMBERREMOVE,
			})
		},
		created() {
			this.open();
