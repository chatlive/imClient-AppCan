	...
	<div id="singleMessage" class="message">
		<div v-if="session">
			<div class="msg-item" v-for="item in session.messages">
				...
				<div v-if="item.from_UserId != user.account">
					{{item.from_UserId}}
				</div>
				<div class="main" :class="{ self: item.from_UserId == user.account }">
					<img class="avatar" width="30" height="30" :src="item.from_UserId == user.account ? user.img : groupFriendImg(item.from_UserId)" />
...
<script>
	...
	export default {
		...
		methods: {
			groupFriendImg(from_UserId) {
				let friends = this.$store.state.imchat.friends;
				let groupFriend = friends.find(friend => friend.account === from_UserId);
				return groupFriend.img;
			},
			groupFriendName(from_UserId) {
				let friends = this.$store.state.imchat.friends;
				let groupFriend = friends.find(friend => friend.account === from_UserId);
			...
			emojicons(msgText) {
				...
				stremo = stremo.replace(reg, function($1) {
					//console.log(`emojicons:${JSON.stringify($1)}`);
					//console.log(`emojicons:${JSON.stringify(emojson.emoarr[$1])}`);
		...
		created() {
			//			let faces = dingdingface.emoarr;
			//			let temp = "";
			//			for(let face of faces) {
			//				temp += `<key>[${face.name}]</key>\r\n<string>${face.src}</string>\r\n`;
			//			}
			//			console.log(temp);
		},
		updated: function() {
